...
module.exports = { keyToScale, makeZipStoreOnly, unzipStoreOnly };
...
/**
 * Create a ZIP file (method 0 - store) from an object of files.
 * @param {Object<string, Uint8Array|string|ArrayBuffer>} filesObj
 * @returns {Uint8Array} ZIP archive containing the provided files
 */
function makeZipStoreOnly(filesObj) {
  ...
  dv3.setUint16(8, count, true);
  dv3.setUint16(10, count, true);
...
/**
 * Unzip a ZIP archive produced with method 0 (store).
 * @param {Uint8Array} u8
 * @returns {Object<string, Uint8Array>} mapping of file names to data
 */
function unzipStoreOnly(u8) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (localMethod !== 0) {
      throw new Error('Unsupported compression method in local header: ' + localMethod);
    }
    const nameLenLocal = view.getUint16(localOffset + 26, true);
    ...
    files[name] = u8.slice(dataStart, dataStart + size);
